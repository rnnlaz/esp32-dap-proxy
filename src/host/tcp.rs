...
impl<'a> TcpChannel<'a> {
    pub fn new(stack: Stack<'a>, port: u16, rx_buffer: &'a mut [u8], tx_buffer: &'a mut [u8]) -> Self {
        ...
        Self { socket, port, deframer: Deframer::new() }
    }
}
...
impl Channel for TcpChannel<'_> {
    async fn accept(&mut self) -> Result<(), Error> {
        self.socket
            .accept(IpListenEndpoint { addr: None, port: self.port })
            .await
    ...
    async fn recv_frame(&mut self, buf: &mut [u8]) -> Result<usize, Error> {
        ...
        loop {
            let n = self.socket.read(&mut chunk).await.map_err(|_| Error::Disconnected)?;
            if n == 0 {
    ...
    async fn send_frame(&mut self, buf: &[u8]) -> Result<(), Error> {
        let head = [FRAME_MAGIC, (buf.len() & 0xFF) as u8, (buf.len() >> 8) as u8];
        self.socket.write_all(&head).await.map_err(|_| Error::Disconnected)?;
        self.socket.write_all(buf).await.map_err(|_| Error::Disconnected)?;
